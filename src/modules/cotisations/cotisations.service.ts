...
export class CotisationsService {
  ...
  // ── Génère un reçu PDF pour une cotisation payée (PAY-005) ───────────────
  async generateRecu(cotisationId: string, currentUser: CurrentUser): Promise<Buffer> {
    const cotisation = await this.prisma.cotisation.findUnique({
      where: { id: cotisationId },
...
throw new BadRequestException("Le reçu n'est disponible que pour une cotisation entièrement payée");    }
    ...
    doc.font('Helvetica').text(
      cotisation.datePaiement ? cotisation.datePaiement.toLocaleDateString('fr-FR') : 'Non précisée',
    );
